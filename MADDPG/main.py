...
if __name__ == '__main__':
    ...
    if isUGV:
        ...
        scenario = 'Multi_UAV_Scenario_Update1'
    #scenario = 'Bach_UAV_Scenario'
    ...
    for i in range(N_GAMES):
        ...
        while (not all(done)) and episode_step < MAX_STEPS:
            ...
            if isUGV:
                for j in range(n_agents):
                    ...
                    #positions[episode_step - 1].append(deepcopy(info['n'][j][6]).tolist())
                    #print('Episode: ', i + 1, ', Episode steps: ', episode_step, pos_in[j])
                    #print(info['n'][j][6])
            else:
                for j in range(n_agents):
                    out_ag_col += (info['n'][j][1])
            ...
            #print('Episode: ', i + 1, ', Episode steps: ', episode_step, pos_in)
            #print('Episode: ', i + 1, ', Episode steps: ', episode_step, positions)
            #positions.append(pos_in)
            #print('Episode: ', i + 1, ', Episode steps: ', episode_step, positions)
        ...
        #print('Yo', positions)
        position_data.append(deepcopy(positions))
        #print(position_data)
        ...
        pos_save_location = 'tmp/run1/pos_data_ep_'
        ...
        if ((i + 1) % 10000 == 0):
            ...
            #dictionary_2 = {'pos_data': position_data}

            with open(save_location + str(i + 1) + '.json', 'w') as outfile:
                json.dump(dictionary_1, outfile)
            #with open(pos_save_location + str(i + 1) + '.json', 'w') as outfile:
                #json.dump(dictionary_2, outfile)
